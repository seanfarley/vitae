...
%
  Worked on adding physics-based preconditioning, new mesh capabilities, and
  advanced time-stepping methods to \bout~(BOUndary Turbulence), a parallel
  edge turbulence framework for plasma fusion simulation. Some work was done to
  improve the build system and software organization as a whole as well. The
  majority of work was improving meshing capabilities to add matrix
  ...
  project competitive performance results. Version 1.0 incorporates my
...
%
  Assisted \blaise~in a graduate class on elliptic solvers ($\Delta u = 0$ for
  convex domains) using previous finite element code from the summer, and
  wrote supporting code in C/C++ using PETSc. This code was extended to work on
  modeling edge detection using the Mumford-Shah functional using phase
  fields. Other duties including helping the \dept's computer support team:
  design of a Kerberos-based authentication system for use with LDAP, http, and
